...
X_simulation <- function(gamma0_prior, tau_prior){
  ...
  prior <- list(0,
                rnorm(1, 0, 1/sqrt(tau + .Machine$double.base)),
                runif(1, -10, 10),
                rnorm(1, 0, 10^(6)),
                rnorm(1, u0, 10^(6)))
  ...
  for (i in 2:k){
    var <- midpoint_distance[i-1]/(tau + .Machine$double.eps)
    # if (var)
...
prior_tau_names <- c('Gamma flat', 'Gamma2', 'PC prior')
...
df_simulations_without_gamma_flat <- df_simulations[df_simulations$tau != "Gamma flat", ]

#Escalas individuais no eixo y
ggplot(data = df_simulations_without_gamma_flat, aes(x = x_value, y = value, 
                                  group = replicate)) +
  scale_x_continuous("Times", expand = c(0,0)) +
  scale_y_continuous("", expand = c(0,0)) +
  geom_line(alpha = .5) + 
