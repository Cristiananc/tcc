...
errors_gamma2_20taxa <- unlist(lapply(reconstructions_20taxa_gamma2, function(x) 100 * (1/date_max) * sum((x$effpop - 10)/10)))
...
widths_gamma2_20taxa <- unlist(lapply(reconstructions_20taxa_gamma2, function(x) (1/date_max) * sum((x$effpop975 - x$effpop025)/10)))
...
errors_gamma2_200taxa <- unlist(lapply(reconstructions_20taxa_gamma2, function(x) 100 * (1/date_max) * sum((x$effpop - 10)/10)))
...
widths_gamma2_200taxa <- unlist(lapply(reconstructions_200taxa_gamma2, function(x) (1/date_max) * sum((x$effpop975 - x$effpop025)/10)))
...
```{r}
ggsave("Imagens/percent_error_constant_trajectory.pdf",
       plot = pe)
```
...
```
```{r}
ggsave("Imagens/width_constant_trajectory.pdf",
       plot = pw)
```
...
summaries_gamma2_200taxa <- do.call(rbind, lapply(1:nrep, function(i){
  data.frame(reconstructions_200taxa_gamma2[[i]]$summary,
             replicate = i, prior = "Gamma2", ntaxa = "200")
}))
...
```
```{r}
ggsave("Imagens/effective_pop_constant_trajectory.pdf",
       plot = peffpop)
```
